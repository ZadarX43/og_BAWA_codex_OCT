#!/usr/bin/env python3
"""Cat+XGB consensus audit for BTTS.

Outputs:
  - BTTS_XGB_CONSENSUS_AUDIT.csv
  - BTTS_XGB_CONSENSUS_AUDIT__BY_LEAGUE.csv
"""
...
def _pick_odds(df: pd.DataFrame, pick_col: str) -> pd.Series:
    ...
    od_yes = pd.to_numeric(df.get("od_yes", df.get("odds_btts_yes")), errors="coerce")
    od_no = pd.to_numeric(df.get("od_no", df.get("odds_btts_no")), errors="coerce")
    return pd.Series(np.where(pick.eq("YES"), od_yes, np.where(pick.eq("NO"), od_no, np.nan)), index=df.index)


def _summarize(df: pd.DataFrame, cohort: str) -> dict:
    d = df.copy()
    d["__pick"] = d["selection"].map(_norm_pick)
    if "actual_btts_yes" in d.columns:
        d["__actual"] = np.where(pd.to_numeric(d["actual_btts_yes"], errors="coerce").eq(1), "YES", "NO")
    else:
        d["__actual"] = None
...
def main() -> None:
    ap = argparse.ArgumentParser(description="BTTS consensus audit (Cat + XGB)")
    ...
    df = df[df["market"].astype(str).str.lower().eq("btts")].copy()
    if "btts_pick_xgb" not in df.columns:
        raise SystemExit("Missing btts_pick_xgb column; re-run scoring after BTTS XGB wiring.")
    if df["btts_pick_xgb"].notna().sum() == 0:
        raise SystemExit("btts_pick_xgb is empty; re-run scoring after BTTS XGB bundle load.")

    df["__consensus"] = (
        df["model_top_pick"].astype("string").fillna("").str.upper().str.strip()
        == df["btts_pick_xgb"].astype("string").fillna("").str.upper().str.strip()
    )
    ...
    out_summary.to_csv(outdir / "BTTS_XGB_CONSENSUS_AUDIT.csv", index=False)
    ...
    pd.DataFrame(by_league).to_csv(outdir / "BTTS_XGB_CONSENSUS_AUDIT__BY_LEAGUE.csv", index=False)
    print(f"Wrote: {outdir / 'BTTS_XGB_CONSENSUS_AUDIT.csv'}")
