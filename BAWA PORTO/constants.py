...
# --- Per-league FTR margin gates (p_top1 - p_top2) -----------------------
# Used by prediction_overlay's FTR candidate selection when enabled.
# Set to 0.0 (or omit league) to disable margin gating for that league.
# NOTE: These are *starting* values intended for placement tests.
FTR_MARGIN_MIN_PER_LEAGUE: dict[str, float] = {
    # Top-tier / stable FTR (keep margin modest)
    "England Premier League": 0.08,
    "Champions League": 0.55,
    "France Ligue 1": 0.08,
    "Europa Conference": 0.00,

    # Near-miss FTR leagues (margin usually improves purity)
    "Portugal Liga": 0.65,
    "Belgium Pro": 0.10,
    "Netherlands Eredivisie": 0.50,

    # Mixed / weaker FTR (raise margin to avoid knife-edge picks)
    "Italy Serie A": 0.55,
    "Germany Bundesliga": 0.28,
    "Brazil Serie A": 0.12,
    "England U21": 0.80,
    "Norway Eliteserien": 0.80,

    # Currently weak / noisy FTR (effectively blocks most picks)
    "Spain La Liga": 0.15,
    "Japan J1": 0.15,
    "Scotland Premiership": 0.18,
    "USA MLS": 0.18,

    # Explicitly excluded / chaotic (kept here for completeness)
    "Europa League": 0.20,
}
...
PER_LEAGUE.setdefault("Italy Serie A", {
    ...
    "FTR_CONF_SHARPEN": "1.45",
    "FTR_PICK_GATE": "0.44",
})
PER_LEAGUE.setdefault("France Ligue 1", {
    "FTR_CAL_MODE": "sigmoid",
    "FTR_CONF_SHARPEN": "1.60",
    "FTR_PICK_GATE": "0.51",
})
PER_LEAGUE.setdefault("Germany Bundesliga", {
    "FTR_CAL_MODE":     "isotonic",
    "FTR_CONF_SHARPEN": "1.48",
    "FTR_PICK_GATE": "0.46",
})
