...
def main() -> None:
    parser = argparse.ArgumentParser(description='Fetch API-Football fixture events into raw JSONL files.')
    ...
    for fixture_id in fixture_ids:
        rows.append(client.get_json('/fixtures/events', {'fixture': fixture_id}))
    stem = source.stem.replace('__fixtures', '') if '__fixtures' in source.stem else source.stem + '__events'
    path = write_raw_json('fixtures_events', rows, stem=stem)
