...
def main() -> None:
    parser = argparse.ArgumentParser(description='Fetch API-Football prematch odds payloads into raw JSONL files using fixture ids.')
    ...
    parser.add_argument('--max-pages-per-fixture', type=int, default=3, help='Safety cap for paginated bookmaker responses.')
    ...
    for fixture_id in fixture_ids:
        pages = client.paged_get('/odds', {'fixture': fixture_id}, max_pages=args.max_pages_per_fixture)
        rows.extend(pages)

    stem = source.stem.replace('__fixtures', '') if '__fixtures' in source.stem else source.stem + '__odds_prematch'
    path = write_raw_json('odds_prematch', rows, stem=stem)
    print(f'WROTE RAW: {path} fixtures={len(fixture_ids)} pages={len(rows)}')
