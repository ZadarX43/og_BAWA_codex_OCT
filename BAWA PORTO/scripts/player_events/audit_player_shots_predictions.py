...
def audit_player_shots_predictions(
    input_csv: str,
    fixtures_csv: str,
    ...
    scored = scored.merge(actual[["fixture_key", "player_name", "shots_total", "shots_on_target", "shots_hit_flag"]], on=["fixture_key", "player_name"], how="left")
    scored["shots_hit_flag"] = scored["shots_hit_flag"].fillna(0).astype(int)
    ...
    for fixture_key, group in scored.groupby("fixture_key", sort=False):
        ...
        actual_high = ranked[ranked["shots_hit_flag"].eq(1)].copy()
        row = {
            ...
            "actual_high_shot_count": int(actual_high["shots_hit_flag"].sum()),
        }
        for n in [1, 3, 5, 10]:
            topn = ranked.head(n)
            row[f"top{n}_fixture_hit_rate"] = int(topn["shots_hit_flag"].sum() > 0)
            row[f"top{n}_precision"] = round(float(topn["shots_hit_flag"].mean()), 4) if len(topn) else 0.0
        rows.append(row)
    ...
    style_df = (
        ...
        .agg(
            ...
            avg_actual_high_shot_count=("actual_high_shot_count", "mean"),
    ...
    summary.to_csv(out_path / (Path(input_csv).stem + f"__shots_audit_summary_last{sample_size}.csv"), index=False)
    style_df.to_csv(out_path / (Path(input_csv).stem + f"__shots_audit_attack_style_last{sample_size}.csv"), index=False)
    fixture_df.to_csv(out_path / (Path(input_csv).stem + f"__shots_audit_fixture_level_last{sample_size}.csv"), index=False)
    return summary, style_df


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit beta player shots predictions.")
    parser.add_argument("--input", required=True)
    parser.add_argument("--fixtures-csv", required=True)
    parser.add_argument("--player-stats-csv", required=True)
    parser.add_argument("--outdir", default="reports/player_events/shots_audits")
...
def main() -> None:
    args = parse_args()
    summary, style_df = audit_player_shots_predictions(
