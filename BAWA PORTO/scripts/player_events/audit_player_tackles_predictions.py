...
def audit_player_tackles_predictions(
    input_csv: str,
    fixtures_csv: str,
    player_stats_csv: str,
    outdir: str,
    sample_size: int = 100,
    tackle_threshold: int = 2,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    inputs = pd.read_csv(input_csv)
    scored = _build_tki(inputs)
    fixtures = pd.read_csv(fixtures_csv, usecols=["fixture_id", "fixture_key", "match_date"])
    actual = pd.read_csv(player_stats_csv, usecols=["fixture_id", "player_name", "tackles", "interceptions"])
    actual = actual.merge(fixtures, on="fixture_id", how="left")
    actual["tackles"] = pd.to_numeric(actual["tackles"], errors="coerce").fillna(0.0)
    actual["tackle_hit_flag"] = (actual["tackles"] >= tackle_threshold).astype(int)
    scored = scored.merge(actual[["fixture_key", "player_name", "tackles", "interceptions", "tackle_hit_flag"]], on=["fixture_key", "player_name"], how="left")
    scored["tackle_hit_flag"] = scored["tackle_hit_flag"].fillna(0).astype(int)
    ...
    for fixture_key, group in scored.groupby("fixture_key", sort=False):
        ranked = group.sort_values("player_tackle_index", ascending=False).reset_index(drop=True)
        actual_high = ranked[ranked["tackle_hit_flag"].eq(1)].copy()
        row = {
            ...
            "actual_high_tackle_count": int(actual_high["tackle_hit_flag"].sum()),
        }
        for n in [1, 3, 5, 10]:
            topn = ranked.head(n)
            row[f"top{n}_fixture_hit_rate"] = int(topn["tackle_hit_flag"].sum() > 0)
            row[f"top{n}_precision"] = round(float(topn["tackle_hit_flag"].mean()), 4) if len(topn) else 0.0
        rows.append(row)

    fixture_df = pd.DataFrame(rows)
    style_df = (
        fixture_df.groupby("fixture_style_label", as_index=False)
        .agg(
            fixtures=("fixture_key", "count"),
            avg_actual_high_tackle_count=("actual_high_tackle_count", "mean"),
    ...
    summary.to_csv(out_path / f"{stem}__tackles_audit_summary_last{sample_size}.csv", index=False)
    style_df.to_csv(out_path / f"{stem}__tackles_audit_style_last{sample_size}.csv", index=False)
    fixture_df.to_csv(out_path / f"{stem}__tackles_audit_fixture_level_last{sample_size}.csv", index=False)
    return summary, style_df


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit beta player tackles predictions.")
    parser.add_argument("--input", required=True)
    parser.add_argument("--fixtures-csv", required=True)
    parser.add_argument("--player-stats-csv", required=True)
    parser.add_argument("--outdir", default="reports/player_events/tackles_audits")
    parser.add_argument("--sample-size", type=int, default=100)
    parser.add_argument("--tackle-threshold", type=int, default=2)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    summary, style_df = audit_player_tackles_predictions(
        input_csv=args.input,
        fixtures_csv=args.fixtures_csv,
        player_stats_csv=args.player_stats_csv,
        outdir=args.outdir,
        sample_size=args.sample_size,
        tackle_threshold=args.tackle_threshold,
    )
