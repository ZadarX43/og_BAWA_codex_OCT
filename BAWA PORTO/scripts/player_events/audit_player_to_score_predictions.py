...
def audit_player_to_score_predictions(
    input_csv: str,
    fixtures_csv: str,
    player_stats_csv: str,
    outdir: str,
    sample_size: int = 100,
    goal_threshold: int = 1,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    inputs = pd.read_csv(input_csv)
    scored = _build_gsi(inputs)
    fixtures = pd.read_csv(fixtures_csv, usecols=["fixture_id", "fixture_key", "match_date"])
    actual = pd.read_csv(player_stats_csv, usecols=["fixture_id", "player_name", "goals"])
    actual = actual.merge(fixtures, on="fixture_id", how="left")
    actual["goals"] = pd.to_numeric(actual["goals"], errors="coerce").fillna(0.0)
    actual["goal_hit_flag"] = (actual["goals"] >= goal_threshold).astype(int)
    scored = scored.merge(actual[["fixture_key", "player_name", "goals", "goal_hit_flag"]], on=["fixture_key", "player_name"], how="left")
    scored["goal_hit_flag"] = scored["goal_hit_flag"].fillna(0).astype(int)
    ...
    for fixture_key, group in scored.groupby("fixture_key", sort=False):
        ranked = group.sort_values("player_goal_index", ascending=False).reset_index(drop=True)
        actual_high = ranked[ranked["goal_hit_flag"].eq(1)].copy()
        row = {
            ...
            "actual_goal_scorer_count": int(actual_high["goal_hit_flag"].sum()),
        }
        for n in [1, 3, 5]:
            topn = ranked.head(n)
            row[f"top{n}_fixture_hit_rate"] = int(topn["goal_hit_flag"].sum() > 0)
            row[f"top{n}_precision"] = round(float(topn["goal_hit_flag"].mean()), 4) if len(topn) else 0.0
        rows.append(row)
    ...
    style_df = (
        ...
        .agg(
            ...
            avg_actual_goal_scorer_count=("actual_goal_scorer_count", "mean"),
    ...
    summary.to_csv(out / (Path(input_csv).stem + f"__goalscorer_audit_summary_last{sample_size}.csv"), index=False)
    style_df.to_csv(out / (Path(input_csv).stem + f"__goalscorer_audit_attack_style_last{sample_size}.csv"), index=False)
    fixture_df.to_csv(out / (Path(input_csv).stem + f"__goalscorer_audit_fixture_level_last{sample_size}.csv"), index=False)
    return summary, style_df


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit beta player-to-score predictions.")
    parser.add_argument("--input", required=True)
    parser.add_argument("--fixtures-csv", required=True)
    parser.add_argument("--player-stats-csv", required=True)
    parser.add_argument("--outdir", default="reports/player_events/goalscorer_audits")
    parser.add_argument("--sample-size", type=int, default=100)
    parser.add_argument("--goal-threshold", type=int, default=1)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    summary, style_df = audit_player_to_score_predictions(
