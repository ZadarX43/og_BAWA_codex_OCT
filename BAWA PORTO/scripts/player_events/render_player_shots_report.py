...
def _shot_role_bonus(df: pd.DataFrame) -> pd.Series:
    ...
    out += role.eq("wide midfielder / winger").astype(float) * 0.07
    out += role.eq("central midfielder").astype(float) * 0.03
    out -= role.eq("holding midfielder").astype(float) * 0.08
    out -= role.eq("centre-back enforcer").astype(float) * 0.18
    out -= role.eq("wide defender / wing-back").astype(float) * 0.10
    out -= role.eq("goalkeeper").astype(float) * 0.30
    return out
...
def _attack_style_bonus(df: pd.DataFrame) -> pd.Series:
    ...
    out += label.eq("CORNER_SIEGE").astype(float) * 0.06
    out += label.eq("TERRITORY_TILT").astype(float) * 0.05
    out += 0.08 * _norm_cap(df.get("fixture_attack_pressure_score", 0.0), 1.0)
    ...
    out += 0.04 * _norm_cap(df.get("og_goal_environment_score", 0.0), 1.0)
    return out


def _quality_attack_bonus(df: pd.DataFrame) -> pd.Series:
    ...
    team_quality = _norm_cap(df.get("starting_xi_team_quality_score", 0.0), 90.0)
...
def _formation_concentration_bonus(df: pd.DataFrame) -> pd.Series:
    ...
    attack_delta = _norm_cap(pd.to_numeric(df.get("lineup_formation_attack_delta", 0.0), errors="coerce").clip(lower=0.0, upper=3.0), 3.0)
    return 0.35 * wide_attack + 0.20 * central_attack + 0.20 * positive_edge + 0.15 * shot_delta + 0.10 * attack_delta


def _build_psi(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    player_volume = (
        0.45 * _norm_cap(out.get("shots_per90", 0.0), 4.0)
        + 0.30 * _norm_cap(out.get("shots_on_target_per90", 0.0), 2.0)
        + 0.10 * _norm_cap(out.get("expected_minutes", 0.0), 90.0)
        + 0.15 * _norm_cap(out.get("fouls_won_per90", 0.0), 3.0)
    )
    team_attack = (
        0.30 * _norm_cap(out.get("home_team_shots_l5", 0.0) + out.get("away_team_shots_l5", 0.0), 30.0)
        + 0.20 * _norm_cap(out.get("home_team_shots_on_goal_l5", 0.0) + out.get("away_team_shots_on_goal_l5", 0.0), 10.0)
        + 0.20 * _norm_cap(out.get("home_team_corners_for_l5", 0.0) + out.get("away_team_corners_for_l5", 0.0), 14.0)
        + 0.15 * _norm_cap(out.get("home_team_possession_l5", 0.0) + out.get("away_team_possession_l5", 0.0), 130.0)
    ...
    out["psi_player_volume"] = player_volume
    out["psi_team_attack"] = team_attack
    out["psi_role_shape"] = role_shape
    out["psi_role_bonus"] = _shot_role_bonus(out)
    out["psi_attack_style_bonus"] = _attack_style_bonus(out)
    out["psi_quality_attack"] = _quality_attack_bonus(out)
    out["psi_formation_concentration"] = _formation_concentration_bonus(out)
    out["player_shot_index"] = (
        0.42 * player_volume
        + 0.28 * team_attack
        + 0.20 * role_shape
        + out["psi_role_bonus"]
        + out["psi_attack_style_bonus"]
        + 0.10 * out["psi_quality_attack"]
        + 0.08 * out["psi_formation_concentration"]
    ) * 100.0
    score = out["player_shot_index"].fillna(0.0)
    out["confidence_label"] = pd.cut(score, bins=[-1, 38, 58, 200], labels=["Low", "Moderate", "High"]).astype("string")
    return out
...
def render_report(df: pd.DataFrame) -> str:
    ...
    ranked = df.sort_values("player_shot_index", ascending=False).copy()
    lines = [
        f"# BAWA Player Shots Prediction: {home} vs. {away}",
        "",
        "## Fixture Overview",
        ...
        f"- corner pressure: {float(pd.to_numeric(pd.Series([first.get('fixture_corner_pressure_score', 0.0)]), errors='coerce').fillna(0.0).iloc[0]):.3f}",
        "",
        "## Ranked Shot Candidates",
    ]
    for idx, row in enumerate(ranked.head(8).itertuples(index=False), start=1):
        lines.extend(
            [
                ...
                f"   - PSI: {row.player_shot_index:.1f}",
                f"   - role: {getattr(row, 'tactical_role', '') or getattr(row, 'position_group', '') or 'n/a'}",
                f"   - shots/90: {getattr(row, 'shots_per90', 'n/a')} | SOT/90: {getattr(row, 'shots_on_target_per90', 'n/a')}",
            ]
        )
    lines.extend(["", "## Final Recommendation", "- beta shortlist only; not a priced shots market model yet", ""])
    return "\n".join(lines)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render a beta player shots report from player-event fixture inputs.")
    parser.add_argument("--input", required=True)
    parser.add_argument("--fixture-key", default="")
    parser.add_argument("--outdir", default="reports/player_events/shots_beta")
...
def main() -> None:
    ...
    scored = _build_psi(fixture_df)
    outdir = Path(args.outdir)
    outdir.mkdir(parents=True, exist_ok=True)
    fixture_key = _string(scored["fixture_key"]).iloc[0]
    safe_key = fixture_key.replace("/", "_").replace(" ", "_")
    out_path = outdir / f"{safe_key}__player_shots_report.md"
    out_path.write_text(render_report(scored))
    print(f"WROTE: {out_path}")
    print(f"rows: {len(scored)}")
    print("top_candidates:", scored.sort_values("player_shot_index", ascending=False)[["player_name", "team_name", "player_shot_index"]].head(5).to_dict(orient="records"))
