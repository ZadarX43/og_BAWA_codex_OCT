...
def _filter_completed_training_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if status_cols:
        ...
        for pat in INCOMPLETE_STATUS_PATTERNS:
            status_bad = status_bad | s.str.contains(pat, regex=True, na=False)
...
def _draw_bundle_joblib_path(league_name: str, model_dir: str = MODEL_DIR) -> str:
    ...
    os.makedirs(model_dir, exist_ok=True)
    # Canonical bundle name: <league_tag>__draw_bundle.joblib (double underscore)
...
SAFE_PREMATCH_WHITELIST = [
    ...
    # team power ratings (long-term Elo-style)
    "home_power_rating", "away_power_rating", "power_diff",
    # defensive pre‑match strength (if available)
    "Home Team Pre-Match xGA", "Away Team Pre-Match xGA",
    # common ETL variants of xGA/xG for/against
    "home_xg_against_avg_overall", "away_xg_against_avg_overall",
    "home_xg_for_avg_overall", "away_xg_for_avg_overall",
    # optional precomputed per‑match defensive rates
...
ALIAS_MAP = {
    ...
    "Home Team Pre-Match xGA": ["home_xg_against_avg_overall", "home_xg_against_avg_home", "home_xg_against_avg_away"],
    "Away Team Pre-Match xGA": ["away_xg_against_avg_overall", "away_xg_against_avg_away", "away_xg_against_avg_home"],
}
...
def train_draw_classifier(training_data, feature_cols, *, league_name: str = "generic", threshold_mode: str | None = None, precision_target: float | None = None, top_k_pcts: list[float] | float = 0.10, model_type: str = "gbm"):
    ...
    try:
        print(
            f"🧷 draw autosave: AUTO_DRAW_THRESHOLD={AUTO_DRAW_THRESHOLD} "
            f"USE_LOCKED_DRAW_THRESHOLDS={USE_LOCKED_DRAW_THRESHOLDS} "
            f"min_samples_required={min_samples_required}"
        )
    except Exception:
        pass
    ...
    SAFE_CORE = {
        ...
        # protect team power ratings from leak-based auto-drop
        "home_power_rating",
        "away_power_rating",
        "power_diff",
    }
    ...
    for tr_idx, va_idx in tss.split(X):
        ...
        if _HAS_IMBLEARN:
            ...
            # Preserve DataFrame + feature names when imblearn returns arrays
            if not isinstance(X_tr_res, pd.DataFrame):
                X_tr = pd.DataFrame(X_tr_res, columns=X_tr.columns)
            else:
                X_tr = X_tr_res
            # Preserve y as a pandas Series aligned to X_tr
            y_tr = pd.Series(y_tr_res)
    ...
    if _HAS_IMBLEARN:
        sampler = RandomOverSampler(random_state=42)
        X_fit_res, y_fit_res = sampler.fit_resample(X_fit, y_fit)
        if not isinstance(X_fit_res, pd.DataFrame):
            X_fit = pd.DataFrame(X_fit_res, columns=X.columns)
        else:
            X_fit = X_fit_res
        y_fit = pd.Series(y_fit_res)
...
def ensure_draw_bundle(df, league: str, row_cap: int | None = None, n_jobs: int = 1, perm_importance: bool = False):
    ...
    try:
        ...
        # Attach team power ratings if the pipeline exposes it
        if hasattr(_pipe, "attach_team_power"):
            df = _pipe.attach_team_power(df, str(league))
    except Exception:
        pass
...
if __name__ == "__main__":
    ...
    def _train_and_score_once():
        ...
        # Attach team power ratings (home/away/power_diff) if available
        try:
            if hasattr(_pipeline, "attach_team_power"):
                df_local = _pipeline.attach_team_power(df_local, str(args.league))
        except Exception as _e:
            print(f"ℹ️  attach_team_power skipped: {_e}")
        print("DEBUG columns after attach_team_power:", df_local.columns.tolist()[:50])
        print("DEBUG any home_power_rating present?", "home_power_rating" in df_local.columns)      
